# backend.py - RENDER UYUMLU, MAX SERTLEŞTİRİLMİŞ
...
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_host=1)  # ProxyFix: doğru remote_addr almak için
...
if ALLOWED_ORIGINS:
    ...
    # production için mümkün olduğunca kısıtlı tut
...
def init_db():
    ...
    Bu fonksiyon Flask uygulama bağlamı olmadan güvenli şekilde çağrılabilir (doğrudan sqlite bağlantısı kullanır).
    """
    # Flask'ın `g` bağlamına bağlı olmayan doğrudan sqlite bağlantısı kullanıyoruz
    conn = sqlite3.connect(DATABASE_PATH, timeout=10, detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    ...
    # Kurucu bilgileri - Sabit kullanıcı (hashed password)
    c.execute("SELECT COUNT(*) FROM users WHERE username = 'admin'")
    if c.fetchone()[0] == 0:
        # generate_password_hash modülü dosyanın başında import edilmiş olmalı
        ...
        c.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', ('admin', hashed, 'admin@' + SITE_DOMAIN if 'SITE_DOMAIN' in globals() else 'admin@example.com', 'admin', 1))
...
SQL_INJECTION_PATTERNS = re.compile(r'\b(select|insert|update|delete|drop|union|--|;)\b', re.I)
...
def security_checks():
    if request.headers.get('X-Forwarded-Proto', request.scheme) != 'https':
        if os.environ.get('REQUIRE_HTTPS', 'true').lower() in ('1','true','yes'):
    ...
    if STRICT_CLOUDFLARE:
        ...
        if cidrs and not ip_in_cidrs(remote, cidrs):
