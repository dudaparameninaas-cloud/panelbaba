...
# If true, only lock Cloudflare for API paths (less likely to block legit users)
...
# Global defaults (web pages). API endpoints get stricter per-route limits below.
limiter = Limiter(
    app=app,
...
# ---------- Middleware / before_request (STABLE) ----------
@app.before_request
def security_checks():
    # allow health always
    ...
    # HTTPS detection (Cloudflare-friendly)
    ...
    # host allowlist (allow onrender preview)
    ...
    # Client IP detection preferring Cloudflare headers
    remote = (
    ...
    # Cloudflare strict mode: only enforce for API paths or when CF_ONLY_API_LOCK is false.
    if STRICT_CLOUDFLARE:
        ...
        if cidrs:
            ...
            if should_enforce:
                # require CF-Ray header to reduce false positives when not proxied
                if request.headers.get('CF-Ray'):
                    if not ip_in_cidrs(remote, cidrs):
                        logger.warning(f"Blocked non-Cloudflare IP for protected path: {remote} path={request.path}")
                        return jsonify({"error": "Access denied"}), 403
                else:
                    # If CF-Ray missing but path is protected, be conservative: log and allow (to avoid lockouts)
                    logger.warning(f"CF-Ray header missing while STRICT_CLOUDFLARE active - allowing request for safety. path={request.path}")
        else:
            # couldn't fetch CF list, don't lock everything
            logger.warning("Cloudflare CIDRs unavailable; skipping CF strict enforcement")

    # Proxy/VPN detection for API endpoints
    if detect_proxy_or_forwarding(request.headers) and request.path.startswith('/api/'):
...
# ---------- Query rate checks (DB-based, stricter thresholds) ----------
def check_user_rate_limits(user_id, user_type):
    ...
    now = datetime.now()
    conn = get_db()
    minute_thresholds = {'free': 3, 'vip': 30}
    hour_thresholds = {'free': 15, 'vip': 500}
    day_thresholds = {'free': 60, 'vip': 2000}
    ...
    min_allowed = minute_thresholds.get(user_type, 3)
    hour_allowed = hour_thresholds.get(user_type, 15)
    day_allowed = day_thresholds.get(user_type, 60)
...
API_ENDPOINTS = {
    # (same list as before — omitted here for brevity in this view but present in actual file)
...
# ---------- STRONG API rate limits applied here ----------
@app.route('/api/execute_query', methods=['POST'])
@login_required
@limiter.limit("5 per minute;200 per hour;1000 per day")
def execute_query():
    try:
        ...
        # Pass client ip to downstream calls
...
@app.route('/health')
def health_check():
    return jsonify({"status":"healthy", "service":"Sorgu Paneli API", "version":"1.2.1", "timestamp": datetime.now().isoformat()})
...
# ---------- Custom Jinja2 filters, error handlers, run ----------
